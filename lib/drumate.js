...
class Drumate extends Common {
   ...
   * 
   */
  removeFromMfs(home_dir) {
    if(this.get('skip_mfs_removal')) return
  ...
  */
  async remove(opt = {}) {
    ...
    if (drumate.id) {
      ...
      let user = await this.yp.await_proc("entity_delete", drumate.id);
      let re = new RegExp(`^${mfs_dir}`);
      if (user.home_dir && re.test(user.home_dir)) {
        rmSync(user.home_dir, { recursive: true, force: true });
      }
    }
  }
