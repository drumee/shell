...
class Organization extends Logger {
  ...
  _vhost() {
    const hosts = [
      'ns1', 'ns2', 'jit', 'www', 'smtp', '_acme-challenge', '_domainkey'
    ];
    let r = []
  ...
  async remove(id) {
    if (!id) {
      this.debug("Error: organization.remove -- require id");
      return []
    }
  ...
  */
  async createAdmin(wp) {
    ...
    if (!lastname) lastname = email_dom || "";
    ...
    await this.yp.await_proc(
      "token_generate_next", email, name, token, FORGOT_PASSWORD, admin.id
    );

    let protocol = this.get('localhost') ? 'http' : 'https';
    let port = this.get('http_port') || this.get('https_port') || "";
    if (port) port = `:${port}`;
  ...
  async updateProgress(type) {
    ...
    this._progress = (this._progress || 0) + 1;
    const model = { progress: this._progress, total: this._totalProgress };
    await RedisStore.sendData({
      service: "sandbox.progress",
      options: { service: "sandbox.progress", type: type || this._typeProgress },
      model
    }, this.socket_id);
  }
  ...
  async createDomain() {
    ...
    while (!isEmpty(dom) || !isEmpty(vhost)) {
      const extra = uniqueNamesGenerator(domainConfig);
      ident = `${extra}-${ident}`;
      fqdn = `${extra}-${name}`;
    ...
    const domain = await this.yp.await_proc("domain_create", ident);
    return { ...domain, ident };
  }

  async createOwner(domain, args = {}) {
    const { name, id, ident } = domain;
    const owner = new Drumate({ yp: this.yp });
    await owner.create({
      domain: name,
    ...
    const rows = await this.yp.await_proc(
      "organisation_create",
      { owner_id: owner.get(Attr.uid), ident },
      { category: "sandbox", ttl: "1 day" }
    );
    let organization = {};
    for (let r of toArray(rows)) {
      if (r && r.failed) {
        await this.remove(id);
        return null;
      }
      if (r.privilege) {
        organization = { ...r, user_domain: r.link };
      }
    }
    ...
    await owner.createHub({ domain: name, area: Attr.dmz }, { filename: uniqueId(), is_wicket: 1 });
    return { ...organization, owner_id: owner.get(Attr.uid) };
  }

  async updateContacts(uid) {
    const { id } = this.domain;
    let peers = [];
    const list = await this.yp.await_proc('member_list_all', uid, id);
